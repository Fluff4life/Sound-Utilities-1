...
function formatAMPM(date) {
    ...
    if(!reason) return msg.reply('I cannot mute the User Without a Reason!')
var channel = msg.guild.channels.cache.find(c => c.id === '');//ADD YOUR log hcannel id here
var log = new Discord.MessageEmbed()
.setColor('RED')
.setTitle("A Member Was Mute")
...
.setColor('YELLOW')
.setTitle(`You were Muted by <@${authorID}> `)
.addField("Server:", '') //delete Sound world and write ur cannel name
.addField("Reason:", `${reason}`)
.addField("Punishment ID:", `\`${punishmentID}\``)
.addField("Time", time)
